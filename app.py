...
try:
    # 从secrets中获取正确的codes
    correct_codes = st.secrets.apikeys
    #st.session_state['logged_in'] = False
    @st.dialog("Enter API key")
    def login():
        with st.expander("Help"):
            st.write("""
            Please enter the API key that you bought at JereShop Premium. If you are a rampion, go to [rampion.streamlit.app](rampion.streamlit.app).""")
        # 创建密码输入框
        user_input_code = st.text_input("Enter the API key you were given.", type="password")
        # 验证用户输入的code是否正确
        if st.button("Get Started"):
            if user_input_code.upper() in correct_codes:
